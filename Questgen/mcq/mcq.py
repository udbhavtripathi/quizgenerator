...
# from main import AnswerPredictor
...
def get_nouns_multipartite(text,num_phrases):
    out = []
    ...
    # keyphrases = extractor.get_n_best(n=num_phrases)
...
#      print("Inside get_keywords function",len(keywords) )
#      keywords = sorted(keywords, key=lambda x: fdist[x])
#      keywords = filter_phrases(keywords, max_keywords,normalized_levenshtein )

#      print("After filter_phrases function",len(keywords) )
...
def get_keywords(nlp,text,max_keywords,s2v,fdist,normalized_levenshtein,no_of_sentences):
    ...
    #  total_phrases = keywords + filtered_phrases
     total_phrases = keywords
    #  total_phrases_filtered = filter_phrases(total_phrases, min(max_keywords, 2*no_of_sentences),normalized_levenshtein )
     total_phrases_filtered = total_phrases
...
def generate_questions_mcq(keyword_sent_mapping,device,tokenizer,model,sense2vec,normalized_levenshtein,num_options):
    batch_text = []
    ...
    encoding = tokenizer.batch_encode_plus(batch_text, pad_to_max_length=True, max_length= 1000, return_tensors="pt")
...
#     print(outs)
    for index, val in enumerate(answers):
        ...
        # answers = AnswerPredictor.predict_answer(payload3)
        ...
        # individual_question["answer"] = answers
        ...
        # if individual_question["answer"] not in individual_question["options"]:
        #     individual_question["options"].append(individual_question["answer"])
